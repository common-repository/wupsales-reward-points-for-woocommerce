...
class FieldWsbp {
	 ...
	 */
	public $adapt = array( 'HtmlWsbp' => '', 'dbFrom' => '', 'dbTo' => '' );
	/**
	...
	public function setValue( $value, $fromDB = false ) {
		if (isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB) {
			$value = FieldAdapterWsbp::_($value, $this->adapt['dbFrom'], FieldAdapterWsbp::DB);
		}
		$this->value = $value;
	}
	...
	public function displayValue() {
		...
		switch ($this->html) {
			case 'checkboxlist':
				$options = $this->getHtmlParam('OptionsWsbp');
				$value = array();
				...
				if (empty($value)) {
					$value = esc_html__('N/A', 'wupsales-reward-points');
				} else {
				...
				if ('' == $this->value) {
					...
					if (is_array($this->value)) {
						...
						if (!empty($options) && is_array($options)) {
							...
							$value = recImplodeWsbp('<br />', $valArr);
						} else {
							$value = recImplodeWsbp('<br />', $this->value);
						}
					} else {
						$value = $this->value;
					}
	...
	public function showValue() {
		HtmlWsbp::echoEscapedHtml($this->displayValue());
	}
	...
	public function checkVarFromParam( $param, $element ) {
		return UtilsWsbp::xmlAttrToStr($param, $element);
	}
	...
	public function prepareConfigOptions( $xml ) {
		...
		foreach ($config->params->param as $param) {
			...
			$helper = $this->checkVarFromParam($param, 'HelperWsbp');
			$module = $this->checkVarFromParam($param, 'ModuleWsbp');
