<?php
abstract class ModelWsbp extends BaseObjectWsbp {
	...
	protected function _buildQuery( $table = null ) {
		if (!$table) {
			$table = FrameWsbp::_()->getTable( $this->_tbl );
		}
	...
	public function removeGroup( $ids ) {
		...
		if (!empty($ids)) {
			if (FrameWsbp::_()->getTable($this->_tbl)->delete(array('additionalCondition' => 'id IN (' . implode(',', $ids) . ')'))) {
				$this->_dataRemove($ids);
				return true;
			} else {
				$this->pushError(esc_html__('Database error detected', 'wupsales-reward-points'));
			}
		} else {
			$this->pushError(esc_html__('Invalid ID', 'wupsales-reward-points'));
		}
		return false;
	}
	...
	public function delete( $params = array() ) {
		if (FrameWsbp::_()->getTable($this->_tbl)->delete($params)) {
	...
	public function insert( $data ) {
		...
		$id = FrameWsbp::_()->getTable($this->_tbl)->insert($data);
		if ($id) {
			return $id;
		}
		$this->pushError(FrameWsbp::_()->getTable($this->_tbl)->getErrors());
		return false;
	}
	public function updateById( $data, $id = 0 ) {
		...
		if ($id) {
			...
			$this->pushError(esc_html__('Empty or invalid ID', 'wupsales-reward-points'));
		}
		return false;
	}
	public function update( $data, $where ) {
		$data = $this->_dataSave($data, true);
		if (FrameWsbp::_()->getTable($this->_tbl)->update($data, $where)) {
			return true;
		}
		$this->pushError(FrameWsbp::_()->getTable($this->_tbl)->getErrors());
		return false;
	...
	public function getFieldLists( $field, $key = false ) {
		$list = UtilsWsbp::getArrayValue($this->fieldLists, $field, array(), 2);
		return false === $key ? $list : UtilsWsbp::getArrayValue($list, $key);
	}
