...
class ResponseWsbp {
	...
	public function ajaxExec( $forceAjax = false ) {
		$isAjax = $this->isAjax();
		$redirect = ReqWsbp::getVar('redirect');
		if (count($this->errors) > 0) {
			$this->error = true;
		}
		if ($isAjax || $forceAjax) {
			HtmlWsbp::echoEscapedHtml(jsonEncodeUTFnormalWsbp($this));
			exit();
		}
		return $this;
	}
	public function mainRedirect( $redirectUrl = '' ) {
		$redirectUrl = empty($redirectUrl) ? WSBP_SITE_URL : $redirectUrl;
		$redirectData = array();
		if (!empty($this->errors)) {
			$redirectData['wsbpErrors'] = $this->errors;
		}
		if (!empty($this->messages)) {
			$redirectData['wsbpMsgs'] = $this->messages;
		}
		return redirectWsbp( $redirectUrl . ( strpos($redirectUrl, '?') ? '&' : '?' ) . http_build_query($redirectData) );
	}
	...
	/**
	 * Alias for ResponseWsbp::addError, @see addError method
	 */
