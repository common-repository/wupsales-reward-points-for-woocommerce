...
abstract class TableWsbp {
	...
	public static function getInstance( $table = '' ) {
		...
		if (!isset($instances[$table])) {
			$class = 'table' . strFirstUpWsbp($table) . strFirstUpWsbp(WSBP_CODE);
	...
	public function isRealTable() {
		return DbWsbp::get("SHOW TABLES LIKE '" . $this->_table . "'");
	}
	public function bulkUpdate( $set, $where ) {
		return DbWsbp::query('UPDATE ' . $this->_table . ' SET ' . $set . ' WHERE ' . $where);
	}
			...
			if ($id) {
				...
				foreach ($data as $field) {
					...
					foreach ($field as $k => $v) {
						if (isset($this->_fields[$k])) {
							$row[$k] = toeCreateObjWsbp('FieldWsbp', array(
									$this->_fields[$k]->name,
									$this->_fields[$k]->html,
									$this->_fields[$k]->type,
	...
	public function getTable( $transform = false ) {
		if ($transform) {
			return DbWsbp::prepareQuery($this->_table);
		} else {
			return $this->_table;
		}
	}
	...
	public function get( $fields = '*', $where = '', $tables = '', $return = 'all' ) {
		...
		return DbWsbp::get($query, $return);
	}
	public function store( $data, $method = 'INSERT', $where = '' ) {
		$this->_clearErrors();
		$method = strtoupper($method);
		if ($this->_escape) {
			$data = DbWsbp::escape($data);
		}
		$query = '';
		switch ($method) {
			case 'INSERT':
		...
		if (empty($fields)) {
			$this->_addError(esc_html__('Nothing to update', 'wupsales-reward-points'));
			return false;
		}
		...
		if (DbWsbp::query($query)) {
			if ('INSERT' == $method) {
				return DbWsbp::lastID();
			} else {
				return true;
			}
		} else {
			$this->_addError(WSBP_TEST_MODE ? DbWsbp::getError() : esc_html__('Database error. Please contact your developer.', 'wupsales-reward-points'));
		}
		return false;
	}
	public function bulkInsert( $data ) {
		...
		if (empty($data)) {
			$this->_addError(esc_html__('Nothing to insert', 'wupsales-reward-points'));
			return false;
		}
		...
		if (!DbWsbp::query(substr($query, 0, -1))) {
			$this->_addError(WSBP_TEST_MODE ? DbWsbp::getError() : esc_html__('Database error. Please contact your developer.', 'wupsales-reward-points'));
			return false;
		}
		return true;
			...
			foreach ($data as $k => $v) {
				if (array_key_exists($k, $this->_fields) || $k == $this->_id) {
					...
					if (isset($this->_fields[$k]) && $this->_fields[$k]->adapt['dbTo']) {
						$val = FieldAdapterWsbp::_($val, $this->_fields[$k]->adapt['dbTo'], FieldAdapterWsbp::DB);
					}
					if ($validate) {
						if (isset($this->_fields[$k]) && is_object($this->_fields[$k])) {
							$objForValidation = clone $this->_fields[$k];
							$objForValidation->setValue($val);
							$errors = ValidatorWsbp::_($objForValidation, $this);
							if ($errors) {
	...
	/**
	 * Add new FieldWsbpWsbp for children table (@see class field)
	 *
	...
	protected function _addField( $name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '' ) {
		$this->_fields[$name] = toeCreateObjWsbp('FieldWsbp', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
		return $this;
	}
	/**
	...
	public function exists( $value, $field = '' ) {
		...
		return DbWsbp::get('SELECT ' . $this->_id . ' FROM ' . $this->_table . ' WHERE ' . $field . ' = "' . $value . '"', 'one');
	}
			...
			switch ($f->type) {
				case 'date':
					if ('0000-00-00' == $d[$key] || empty($d[$key])) {
						...
						$d[$key] = gmdate(WSBP_DATE_FORMAT, DbWsbp::dateToTime($d[$key]));
					}
					break;
				case 'int':
				case 'tinyint':
	...
	public function getLastInsertID() {
		return DbWsbp::get('SELECT MAX(' . $this->_id . ') FROM ' . $this->_table, 'one');
	}
