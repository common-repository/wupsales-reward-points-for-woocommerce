...
class UtilsWsbp {
	...
	public static function jsonEncode( $arr, $ent = false ) {
		return ( is_array($arr) || is_object($arr) ) ? jsonEncodeUTFnormalWsbp($arr, $ent) : jsonEncodeUTFnormalWsbp(array());
	}
	...
	public static function isThisCommercialEdition() {
		foreach (FrameWsbp::_()->getModules() as $m) {
	...
	public static function isMobile() {
		importClassWsbp('Mobile_Detect', WSBP_HELPERS_DIR . 'mobileDetect.php');
	...
	public static function isPluginsPage() {
		return ( basename(ReqWsbp::getVar('SCRIPT_NAME', 'server')) === 'plugins.php' );
	}
	...
	public static function getReferalUrl() {
		// Simple for now
		return ReqWsbp::getVar('HTTP_REFERER', 'server');
	}
	...
	public static function getArrayValue( $params, $name, $default = '', $typ = 0, $arr = false, $zero = false ) {
		if (!isset($params[$name])) {
			return $default;
		}
		if (empty($params[$name])) {
			return ( $zero && ( '0' === $params[$name] || 0 === $params[$name] ) ) ? 0 : $default;
		}
		$value = $params[$name];
		if (1 == $typ) {
			if (!is_numeric($value)) {
	...
	public static function getJSDateFormat( $format = '' ) {
		if (empty($format)) {
			$format = FrameWsbp::_()->getModule('bonuses')->getDateFormat();
		}
	...
	public static function getFormatedDateTime( $dt, $format = '' ) {
		if (empty($format)) {
			$format = FrameWsbp::_()->getModule('bonuses')->getDateFormat() . ' H:i';
		}
	...
	public static function convertDateFormat( $dt, $from = '', $to = 'Y-m-d' ) {
		...
		if (empty($from)) {
			$from = FrameWsbp::_()->getModule('bonuses')->getDateFormat();
		}
		if ($from == $to) {
			return $dt;
		}
