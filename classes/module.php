<?php
abstract class ModuleWsbp extends BaseObjectWsbp {
	...
	public function __construct( $d) {
		...
		if (isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
			...
			$this->setExternalDir( UtilsWsbp::getExtModDir($d['ex_plug_dir']) );
			$this->setExternalPath( UtilsWsbp::getExtModPath($d['ex_plug_dir']) );
		}
	}
	...
	public function getModDir() {
		if (empty($this->_externalDir)) {
			return WSBP_MODULES_DIR . $this->getCode() . DS;
		} else {
	...
	public function getModPath() {
		if (empty($this->_externalPath)) {
			return WSBP_MODULES_PATH . $this->getCode() . '/';
		} else {
	...
	protected function _createController() {
		...
		if (file_exists($this->getModDir() . 'controller.php')) {
			$className = '';
			if (importWsbp($this->getModDir() . 'controller.php')) {
				$className = toeGetClassNameWsbp($this->getCode() . 'Controller');
			}
			if (!empty($className)) {
	...
	protected function _createHelper() {
		...
		if (file_exists($this->getModDir() . 'helper.php')) {
			...
			importClassWsbp($helper, $this->getModDir() . 'helper.php');
	...
	public function translate( $str ) {
		return esc_html__($str, 'wupsales-reward-points');
	}
}
