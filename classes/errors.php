...
class ErrorsWsbp {
	...
	public static function setSession( $error ) {
		...
		ReqWsbp::setVar('sesErrors', $sesErrors, 'session');
	}
	public static function init() {
		$wsbpErrors = ReqWsbp::getVar('wsbpErrors');
		if (!empty($wsbpErrors)) {
			if (!is_array($wsbpErrors)) {
				$wsbpErrors = array( $wsbpErrors );
			}
			$wsbpErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $wsbpErrors)));
			if (!empty($wsbpErrors)) {
				self::$current = $wsbpErrors;
				if (is_admin()) {
					add_action('admin_notices', array('ErrorsWsbp', 'showAdminErrors'));
				} else {
					add_filter('the_content', array('ErrorsWsbp', 'appendErrorsContent'), 99999);
				}
			}
		}
	}
	...
	public static function getSession() {
		return ReqWsbp::getVar('sesErrors', 'session');
	}
	public static function clearSession() {
		ReqWsbp::clearVar('sesErrors', 'session');
	}
