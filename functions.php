 ...
 */
if (!function_exists('strFirstUpWsbp')) {
	function strFirstUpWsbp( $str ) {
 ...
 */
if (!function_exists('dateToTimestampWsbp')) {
	function dateToTimestampWsbp( $date ) {
		if (empty($a)) {
			return false;
		}
		$a = explode(WSBP_DATE_DL, $date);
 ...
 */
if (!function_exists('getRandNameWsbp')) {
	function getRandNameWsbp( $lenFrom = 6, $lenTo = 9 ) {
		$res = '';
...
if (!function_exists('importWsbp')) {
	function importWsbp( $path ) {
		if (file_exists($path)) {
			require($path);
			return true;
		}
...
if (!function_exists('setDefaultParamsWsbp')) {
	function setDefaultParamsWsbp( $params, $default ) {
...
if (!function_exists('importClassWsbp')) {
	function importClassWsbp( $class, $path = '' ) {
		if (!class_exists($class)) {
			if (!$path) {
				$classFile = lcfirst($class);
				if (strpos(strtolower($classFile), WSBP_CODE) !== false) {
					$classFile = preg_replace('/' . WSBP_CODE . '/i', '', $classFile);
				}
				$path = WSBP_CLASSES_DIR . $classFile . '.php';
			}
			return importWsbp($path);
		}
		return false;
	}
}
 ...
 */
if (!function_exists('toeGetClassNameWsbp')) {
	function toeGetClassNameWsbp( $class ) {
		$className = '';
		if (class_exists($class . strFirstUpWsbp(WSBP_CODE))) {
			$className = $class . strFirstUpWsbp(WSBP_CODE);
		} else if (class_exists(WSBP_CLASS_PREFIX . $class)) {
			$className = WSBP_CLASS_PREFIX . $class;
		} else {
			$className = $class;
		}
		return $className;
 ...
 */
if (!function_exists('toeCreateObjWsbp')) {
	function toeCreateObjWsbp( $class, $params ) {
		$className = toeGetClassNameWsbp($class);
		$obj = null;
 ...
 */
if (!function_exists('redirectWsbp')) {
	function redirectWsbp( $url ) {
		if (headers_sent()) {
...
if (!function_exists('jsonEncodeUTFnormalWsbp')) {
	function jsonEncodeUTFnormalWsbp( $value, $ent = false ) {
		if (is_int($value)) {
			return (string) $value;   
		...
		if ($with_keys) {
			foreach ($value as $key => $v) {
				$result[] = jsonEncodeUTFnormalWsbp((string) $key, $ent) . ':' . jsonEncodeUTFnormalWsbp($v, $ent);    
			}
			return '{' . implode(',', $result) . '}';                
		} else {
			foreach ($value as $key => $v) {
				$result[] = jsonEncodeUTFnormalWsbp($v, $ent);    
			}
			return '[' . implode(',', $result) . ']';
		}
	} 
 ...
 * @return array
 */
if (!function_exists('prepareParamsWsbp')) {
	function prepareParamsWsbp( &$d = array(), &$options = array() ) {
		if (!empty($d['params'])) {
			if (isset($d['params']['options'])) {
				$options = $d['params']['options'];
			}
			if (is_array($d['params'])) {
				$params = UtilsWsbp::jsonEncode($d['params']);
		...
		if (isset($d['code'])) {
			if ('' == $d['code']) {
				$d['code'] = prepareFieldCodeWsbp($d['label']) . '_' . rand(0, 9999999);
			}
		}
		return $d;
	}
}
if (!function_exists('prepareFieldCodeWsbp')) {
	function prepareFieldCodeWsbp( $string ) {   
 ...
 */
if (!function_exists('recImplodeWsbp')) {
	function recImplodeWsbp( $glue, $array ) {
		$res = '';
		$i = 0;
		$count = count($array);
		foreach ($array as $el) {
